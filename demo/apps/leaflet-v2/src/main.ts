...
app.innerHTML = renderMapDemoPage({
  routeLabel: isDevelopment ? (isLocalSource ? 'Leaflet 2.x local route' : 'Leaflet 2.x npm route') : 'Leaflet 2.x',
  subtitle:
    isDevelopment
      ? isLocalSource
      ...
      : 'Evaluation route for teams exploring the Leaflet 2 path.',
  homeHref,
